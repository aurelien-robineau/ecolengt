...
| *À compléter après inventaire (souvent 5–15 lignes, pas 100)* | … |
...
1. `site:votre-domaine.fr` + navigation manuelle + Wayback Machine → liste courte des URLs à préserver  
2. Tableau **ancienne → nouvelle** URL (souvent une dizaine de lignes) + redirections 301 + redéploiement  
3. Domaine sur Vercel + `NEXT_PUBLIC_SERVER_URL` + redirection www / sans www  
